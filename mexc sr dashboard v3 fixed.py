#!/usr/bin/env python3
"""
MEXC Stock Futures S/R Dashboard v2 — Maximum Quality Filters
=============================================================
Every practical tightening applied for higher-quality setups.

Tightenings included:
- Strict multi-TF confluence (Daily zone + 4H alignment or HVN)
- Trend / regime filter (SMA50 + rising, plus SPY/QQQ bias)
- Volume confirmation (>1.4× average on signal bar)
- Candle rejection / close strength confirmation
- Tighter distance-to-zone (< 0.55 ATR)
- Higher minimum strength + quality score threshold
- Raised minimum R:R (2.2+)
- Only top-ranked setups shown
- Market regime awareness (risk-on / risk-off via SPY)
- Distance monitoring & “near zone” highlighting
- Enhanced Plotly charts (SMAs, clearer zones, entry annotations)
- Direct links to Yahoo news / chart for each symbol

Run:
  streamlit run mexc_sr_dashboard_v2.py --server.port 8501
"""
...
def score_zones(zones: List[Zone], df: pd.DataFrame, sig: Optional[Tuple[float, float]]) -> List[Zone]:
    ...
    for z in zones:
        ...
        if z.source == "hvn":
            z.strength *= 1.35
        scored.append(z)
    scored = [z for z in scored if z.touches >= MIN_TOUCHES or z.strength > 2.5 or z.source == "hvn"]
...
def candle_rejection(df: pd.DataFrame, side: str) -> bool:
    ...
    rng = h - l + 1e-9
    if side == "long":
        # lower wick dominant or strong close near high
...
def generate_entries(zones: List[Zone], last: float, atr: float, df: pd.DataFrame,
    ...
    for z in strong:
        ...
        if z.kind in ("support", "both") and last >= z.low - atr * 0.15:
            if last > z.high + atr * 0.8:  # already too far above
                continue
            # Trend filter
            ...
            # Quality score (0–12-ish)
            q = 0.0
            q += min(z.strength / 2.0, 3.5)
            q += 2.0 if z.source in ("hvn", "both") else 0.0
    ...
    return ideas[:2]  # only the very best
...
def analyze(futures: str, equity: str, market_bias: str) -> AnalysisResult:
    try:
        df_d = fetch_ohlcv(equity, "6mo", "1d")
        if len(df_d) < 70:
            return AnalysisResult(futures, equity, 0, 0, 0, 0, False, error="Insufficient data")
        ...
        sma_rising = sma50_s.iloc[-1] > sma50_s.iloc[-6]

        # Daily swings + HVN
        h_idx, h_p, l_idx, l_p = find_swings(df_d, atr_d_s, PEAK_DIST_D)
        vol = df_d["Volume"].values
        eps = max(last * ZONE_EPS_PCT, atr_d * ZONE_EPS_ATR)
        ...
        if df_4h is not None and len(df_4h) > 50:
            atr4_s = atr(df_4h)
            atr_4h = float(atr4_s.iloc[-1])
            h4, hp4, l4, lp4 = find_swings(df_4h, atr4_s, PEAK_DIST_4H)
            v4 = df_4h["Volume"].values
            eps4 = max(last * ZONE_EPS_PCT * 0.9, atr_4h * ZONE_EPS_ATR)
        ...
        zones = sorted(zones, key=lambda z: -z.strength)[:12]

        # Nearest zone distance
        nearest = 99.0
        for z in zones:
            d = abs(last - z.center) / atr_d
            if d < nearest:
                nearest = d

        entries = generate_entries(zones, last, atr_d, df_d, sma50, sma_rising, market_bias)

        return AnalysisResult(
            futures=futures, equity=equity, last=last, atr_d=atr_d, atr_4h=atr_4h,
            sma50=sma50, sma50_rising=sma_rising, zones=zones, entries=entries,
            df_d=df_d, nearest_zone_dist_atr=nearest, market_bias=market_bias
        )
    except Exception as e:
        return AnalysisResult(futures, equity, 0, 0, 0, 0, False, error=str(e))
...
def make_chart(res: AnalysisResult) -> go.Figure:
    ...
    fig.update_layout(
        title=f"{res.futures} ({res.equity})  Last {res.last:.2f}  |  ATR {res.atr_d:.2f}  |  Bias {res.market_bias}  |  Near {res.nearest_zone_dist_atr:.2f} ATR",
...
if run_btn or (auto and (st.session_state.last_run_v2 is None or
    ...
    if not selected:
        ...
        for i, sym in enumerate(selected):
            ...
            results[sym] = analyze(sym, UNIVERSE[sym], bias)
            time.sleep(0.4)
...
for sym, res in results.items():
    if res.error:
        rows.append({"Symbol": sym, "Last": "—", "Q": "", "Side": "ERR", "Entry": res.error[:35],
                     "Stop": "", "Target": "", "R:R": "", "Dist": "", "Size (coins)": "", "Notional": "", "Reason": ""})
        continue
    if not res.entries:
        near = f"{res.nearest_zone_dist_atr:.2f}" if res.nearest_zone_dist_atr < 10 else "—"
        rows.append({"Symbol": sym, "Last": f"{res.last:.2f}", "Q": "", "Side": "—", "Entry": "No high-Q setup",
...
# Optional simple webhook ping for high-Q setups
...
if res.error:
    ...
    c1.metric("Last", f"{res.last:.2f}")
    c2.metric("ATR", f"{res.atr_d:.2f}")
    c3.metric("SMA50", f"{res.sma50:.2f}", delta="rising" if res.sma50_rising else "falling")
    c4.metric("Nearest Zone", f"{res.nearest_zone_dist_atr:.2f} ATR")
    c5.metric("Bias", res.market_bias)

    fig = make_chart(res)
    st.plotly_chart(fig, use_container_width=True)

    # News / external links (reliable sources)
