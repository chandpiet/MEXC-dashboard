...
def detect_fair_value_gaps(df: pd.DataFrame, atr_val: float, tf: str = "1D",
    ...
    Only keep gaps that are still unfilled (or only partially filled) and
    ...
    for i in range(2, n):
        ...
        if highs[i - 2] < lows[i]:
            ...
            if gap_size < atr_val * min_gap_atr:
                continue
            # Check if later price has fully filled it
            filled = False
...
def score_zones(zones: List[Zone], df: pd.DataFrame, sig: Optional[Tuple[float, float]]) -> List[Zone]:
    ...
    for z in zones:
        ...
        if z.source == "hvn":
            z.strength *= 1.35
        if z.source == "fvg":
            # FVGs get a solid base strength; size already filtered by ATR
